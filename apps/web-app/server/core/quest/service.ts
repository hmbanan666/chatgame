...
export class ViewerQuestService {
  ...
  async tryAssignQuest(profileId: string, userName: string): Promise<void> {
    ...
    const text = template.textRu.replace('{goal}', String(goal))
  ...
  async #completeQuest(quest: ActiveQuest): Promise<void> {
    try {
      ...
      logger.info(`Quest completed: ${quest.templateId} by profile ${quest.profileId}, +${quest.reward} coins`)
    } catch (err) {
