import type { GameObject, GameScript, GameScriptTask, MovementTarget } from '../types'
...
export class BaseScript implements GameScript {
  id: GameScript['id']
  tasks: GameScript['tasks'] = []
  isInterruptible: GameScript['isInterruptible'] = false
  ...
  markTaskAsActive(task: GameScriptTask) {
    task.status = 'ACTIVE'
  }
  ...
  setTarget(target: MovementTarget): GameScriptTask {
    return {
      id: createId(),
      status: 'IDLE',
      live: () => {
  ...
  runToTargetAndDestroy(): GameScriptTask {
    return {
      id: createId(),
      status: 'IDLE',
      live: () => {
