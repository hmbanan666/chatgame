<template>
  <div class="my-16 px-4 max-w-4xl mx-auto text-center space-y-6">
    <div class="mb-8 space-y-4">
      <h1 class="text-2xl md:text-3xl lg:text-4xl">
        Поддержать стримера
      </h1>
      <p class="text-base md:text-lg lg:text-xl">
        Какой способ не выбери - спасибо! Больше мотивации делать крутые штуки.
      </p>
    </div>

    <ActiveCard class="py-4 px-6 grid grid-cols-1 md:grid-cols-3 gap-y-4 md:gap-x-4 items-center justify-center text-site-bg-alt">
      <p class="col-span-2">
        Укажи любую сумму и напиши сообщение, которое появится на активном стриме. И даже будет озвучка.
      </p>
      <a
        href="https://www.donationalerts.com/r/hmbanan666"
        target="_blank"
        class="px-6 py-5 w-full bg-[#E6904E] border-b-6 border-[#CD683D] text-white text-base font-semibold tracking-wide rounded-lg cursor-pointer hover:opacity-85 active:scale-95 duration-200 flex flex-row justify-center items-center gap-3 flex-wrap"
      >
        <Icon name="simple-icons:twitch" class="size-8!" />
        <p class="uppercase">Donationalerts</p>
      </a>
    </ActiveCard>

    <ActiveCard class="py-4 px-6 grid grid-cols-1 md:grid-cols-3 gap-y-4 md:gap-x-4 items-center justify-center text-site-bg-alt">
      <p class="col-span-2">
        Приобрети любой набор Монет для игры. Оплата через "ЮKassa": РФ карта, ЮMoney, SberPay.
      </p>
      <a href="/#shop" class="px-6 py-5 w-full bg-[#0EAF9B] border-b-6 border-[#0B8A8F] text-white text-lg font-semibold tracking-wide rounded-lg cursor-pointer hover:opacity-85 active:scale-95 duration-200 flex flex-row justify-center items-center gap-3 flex-wrap">
        <p>Купить Монеты</p>
      </a>
    </ActiveCard>
  </div>
</template>
